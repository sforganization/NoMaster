...
#define     FINGER_ACK_ADDR         0xFF  //指纹模块应答地址
...
typedef struct 
{
    ...
    WatchState_t    WatchState[16];                     //最大16个手表
    WatchState_t    WatchStateSave[16];                 //最大16个手表
    ...
    MotoState       Moto_State[GLASS_MAX_CNT];       //电机01运行状态
    MotoSubState    Moto_SubState[GLASS_MAX_CNT];    //电机_01运行子状态
    MotoFR          Moto_Mode[GLASS_MAX_CNT];        //电机_01模式
    MotoFR          Moto_ModeSave[GLASS_MAX_CNT];    //电机_01模式保存
    MotoTime_e      Moto_Time[GLASS_MAX_CNT];        //电机_01运行时间枚举
    vu32 			Moto_RunTime[GLASS_MAX_CNT];		//_01电机运行时间
    vu32 			Moto_WaitTime[GLASS_MAX_CNT];		//_01电机停止时间
